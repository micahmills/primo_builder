<script>
	...
	$: filtered_available_languages = ((text) => {
		...
		function match(language) {
			...
			try {
				matching_language =
					language.key.match(text.toLowerCase()) ||
					language.name.toLowerCase().match(text.toLowerCase())
			} catch (e) {
...
>
	...
	>
		<svg width="13" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
		...
		<svg id="chevron" width="10" viewBox="0 0 12 15" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
					...
					{#each site_languages.sort((l) => (l === $primary_language ? -1 : 1)) as languageID}
						<div class="language-item">
							<button
								...
								class="option"
								class:active={languageID === $locale}
							>
			...
			{:else}
				<div class="search-container">
					...
					<div class="language-list">
						{#each filtered_available_languages as loc}
							<button
								on:click={() => {
									add_language(loc.key)
									addingLanguage = false
								}}
								class="option"
