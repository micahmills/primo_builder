...
<style>
	...
	.repeater-item .item-options .primo-buttons {
				white-space: nowrap;
			}

	.repeater-item .item-options:not(:only-child) {
				border-bottom: var(--input-border);
				padding-bottom: 0.75rem;
			}

	.repeater-item .item-options button.title {
				padding: 0.5rem 0;
				display: flex;
				gap: 1rem;
				align-items: center;
	...
	.repeater-item .item-options button.title img {
					width: 3rem;
					border-radius: 2px;
				}

	.repeater-item .item-options .primo-buttons button:focus {
					/* outline: 0; */
				}

	.repeater-item .item-options .primo-buttons button:hover {
					color: var(--primo-color-brand);
				}

	.repeater-item .item-options .primo-buttons button:last-child {
					margin-left: 0.5rem;
					color: var(--color-gray-5);
				}

	.repeater-item .item-options .primo-buttons button:last-child:hover {
						color: var(--primo-color-brand);
					}

	.repeater-item .field-values {
			display: grid;
			gap: 2rem;
		}

	.repeater-item .field-values .repeater-label {
				display: block;
				font-size: var(--title-font-size);
				font-weight: var(--title-font-weight);
				margin-bottom: 1rem;
